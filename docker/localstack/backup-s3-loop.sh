...
BACKUP_INTERVAL_SECONDS=${S3_BACKUP_INTERVAL_SECONDS:-10}
mkdir -p "$BACKUP_ROOT"
...
sync_once() {
  local bucket_names bucket_name bucket_dir
...
) >/var/lib/localstack/logs/s3-backup-loop.log 2>&1 &

echo "S3 backup loop started with interval ${BACKUP_INTERVAL_SECONDS}s."
